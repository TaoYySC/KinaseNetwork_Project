...
def fit_model(data, prior, output_path='./test_result', 
              ...
              eps=torch.finfo(torch.float).eps, eps_factor=10, fill_zeroed=True, device='cpu', alpha = 0.7):
    """
    ...
    for cv in range(cvs):
        ...
        for wd in weight_decays:
            ...
            fit_report = trainer.run_epochs(optimizer=opt_setter.optimizer, model=model, train_loader=train_loader, 
                                            val_loader=val_loader, cv=cv, weight_decay=wd, refit=None, alpha = alpha)
            ...
            refit_model(model=model, data_spliter=data_spliter, trainer=trainer, evaluator=evaluator, 
                        ...
                        lr=lr, wd=wd, relmax=0, cv=cv, output_path=output_path, device=device, alpha = alpha)
...
def refit_model(model, data_spliter, trainer, evaluator, train_loader, val_loader, 
                ...
                relmax=0, cv=0, output_path='./test_result', device='cpu', alpha = 0.7):
    """
